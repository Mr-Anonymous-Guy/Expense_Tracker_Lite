...
import { retrieveContext, type KnowledgeChunk } from "./rag.js";
...
export async function analyzeExpenses(payload: z.infer<typeof financePayloadSchema>) {
...
export async function adviseBudget(payload: z.infer<typeof financePayloadSchema>) {
  ...
  const result = await prompt.pipe(model).invoke({
    context: context.map((item: KnowledgeChunk) => `${item.title}: ${item.content}`).join("\n"),
    data: JSON.stringify(payload)
  });
  return String(result.content);
}

export async function chatWithCopilot(payload: z.infer<typeof financePayloadSchema>) {
